...
// Назначение: подтверждение/отмена pending-записей
...
export function BookingsPage() {
  ...
  return (
    <div className="admin">
      ...
      {items.length === 0 && <div className="admin__muted">Новых записей нет</div>}

      {items.map((b) => (
        <div key={b.id} className="admin-card admin-card--pending">
          <div className="admin-card__head">
            <div className="admin-card__title">{b.service.name}</div>
            <div className="badge badge--pending">ожидает</div>
          </div>
